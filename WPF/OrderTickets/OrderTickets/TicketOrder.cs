    ...
    class TicketOrder
    {
        ...
        public string CustomerReference
        {
            ...
            set
            {
                if (String.IsNullOrEmpty(value))
                {
                    throw new ApplicationException("Specify the customer reference number");
                }
                else
                {
        ...
        public PrivilegeLevel PrivilegeLevel
        {
            ...
            set
            {
                ...
                if (!checkPrivilegeAndNumberOfTickets(value, numberOfTickets))
                {
                    throw new ApplicationException("Privilege level too low for this number of tickets");
                }
            }
        }
        ...
        public short NumberOfTickets
        {
            ...
            set
            {
                ...
                if (!checkPrivilegeAndNumberOfTickets(privilegeLevel, value))
                {
                    throw new ApplicationException("Too many tickets for this privilege level");
                }
                if (numberOfTickets <= 0)
                {
                    throw new ApplicationException("You must buy at least one ticket!");
                }
            }
        }

        private bool checkPrivilegeAndNumberOfTickets(PrivilegeLevel proposedPrivilegeLevel, short proposedNumberOfTickets)
        {
            bool retVal = false;
        ...
        {
            return $"Event: {eventName}\tCustomer: {customerReference}\tPrivilege: {privilegeLevel}\tTickets: {numberOfTickets}";
        }
    }
